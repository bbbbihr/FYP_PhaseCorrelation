clear;
defocus_length=zeros(1,90);
array=zeros(3,90);
size0=0.1;
for k=1:90
    defocus_length(k)=-100+2*(k-1);
end
phase_correlation=zeros(1,90);
time=0;
for i=1:90
    t1=clock;
    s1=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_20x_RFtarget\\_AfBmpB_%d_%dum.png",i-1, defocus_length(i));
    s2=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_20x_RFtarget\\_AfBmpA_%d_%dum.png",i-1, defocus_length(i));
    image1 = imread(s1);
    image2 = imread(s2);
    ...
    [y x]=find(result==max(max(result))); x=x(1); y=y(1);
    %offset
   if x>640*size0
        x=(1280*size0)-x;
    end
