clear;
defocus_length=zeros(1,61);
size0=0.1;
array=zeros(3,90);
for k=1:61
    defocus_length(k)=-150+5*(k-1);
end
phase_correlation=zeros(1,61);
for i=1:61
    t1=clock;
    s1=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_10x_RFtarget\\_AfBmpB_%d_%dum.png",i-1, defocus_length(i));
    s2=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_10x_RFtarget\\_AfBmpA_%d_%dum.png",i-1, defocus_length(i));
    image1 = imread(s1);
    image2 = imread(s2);
