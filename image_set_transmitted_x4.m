clear;
defocus_length=zeros(1,281);
size0=0.1;
for k=1:281
    defocus_length(k)=-1400+10*(k-1);
end
phase_correlation=zeros(1,281);
for i=1:281
    t1=clock;
    s1=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\Transmitted_4x_BF_RF\\LED A\\_AfBmpA_%d_%dum.png",i-1, defocus_length(i));
    s2=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\Transmitted_4x_BF_RF\\LED B\\_AfBmpB_%d_%dum.png",i-1, defocus_length(i));
    image1 = imread(s1);
    image2 = imread(s2);
