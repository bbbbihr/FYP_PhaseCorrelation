clear;
defocus_length=zeros(1,101);
size0=0.25;
array=zeros(3,101);
for k=1:101
    defocus_length(k)=-50+(k-1);
end
phase_correlation=zeros(1,101);
for i=1:101
    s1=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_40x_RFtarget_attempt_2\\_AfBmpB_%d_%dum.png",i-1, defocus_length(i));
    s2=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\episcopic_40x_RFtarget_attempt_2\\_AfBmpA_%d_%dum.png",i-1, defocus_length(i));
    image1 = imread(s1);
    image2 = imread(s2);
...
%     result = ordfilt2(result,1,ones(5,5));%minimum filter
    ...
    array(1,i)=i;array(2,i)=x;array(3,i)=y;
end
