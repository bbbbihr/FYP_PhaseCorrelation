...
size0=0.2;
for k=1:61
    defocus_length(k)=-30+1*(k-1);
end
phase_correlation=zeros(1,61);
for i=1:61
    t1=clock;
    s1=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\Transmitted_40x_BF_RF\\_AfBmpA_%d_%dum.png",i-1, defocus_length(i));
    s2=sprintf("F:\\Final year project\\Test images\\PreFACE raw data stacks\\Transmitted_40x_BF_RF\\_AfBmpB_%d_%dum.png",i-1, defocus_length(i));
    image1 = imread(s1);
    image2 = imread(s2);
